...
export async function GET(req: NextRequest) {
  ...
  if (!existing || existing.length === 0) {
    await supabase.from('material_slots').insert(
      DEFAULT_SLOTS.map(slot_time => ({ meeting_id: meetingId, slot_time, max_teams: 5 }))
    )
  }
...
// POST /api/material-slots/reserve — 예약
export async function POST(req: NextRequest) {
  ...
  if (error) {
    if (error.code === '23505') {
      return NextResponse.json({ error: '이미 해당 시간대에 신청하셨습니다' }, { status: 409 })
    }
